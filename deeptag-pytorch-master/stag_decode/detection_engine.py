...
class DetectionEngine:
    ...
    def process(self, image, detect_scale = None):
        ...
        # print(self.bbox_corner_info)
        return decoded_tags

    # Patro - obter informações
    def getInfo(self, image, detect_scale = None) -> str:
        stag_detector = self.stag_detector
        texto = ""
        texto += str(self.rois)
        return 
    ...
    def visualize(self, is_pause = True):
        ...
        for decoded_tag in decoded_tags:
            ...
            visualize_rt(image_out_pose, decoded_tag['rvecs'], decoded_tag['tvecs'], self.cameraMatrix, self.distCoeffs, decoded_tag['tag_real_size_in_meter'], tag_id_decimal = tag_id_decimal, tid_text_pos=None, score=None, is_draw_cube =True, rotate_idx=rotate_idx, text_color= None)
