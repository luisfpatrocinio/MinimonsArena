...
if __name__ == "__main__":
    ...
    parser.add_argument('--CPU', action='store_true', help='usef CPU only')
    ...
    if load_config_flag and load_codebook_flag and load_model_flag:
        ...
        while True:
            ...
            if is_video:
                if cap is None: cap = cv2.VideoCapture(1)
                ret, image =cap.read()
                if not ret:
            ...
            # info = stag_image_processor.getInfo()
            # print(info)
            ...
            if len(decoded_tags) > 0:
                # RPY - Roll Pitch Yam
                validTags = list(filter(lambda _tag: _tag["is_valid"] == True, decoded_tags))
                print("# Tags válidas: ", len(validTags))

                for tag in validTags:
                    
                    msg += "tag:" + str(tag["tag_id"]) + "$"

                    rvecs = np.array(tag["rvecs"])
                    tvecs = np.array(tag["tvecs"])
                    
                    # Convertendo os arrays em strings
                    rvecs_str = np.array2string(rvecs, separator=',').replace("\n", "")
                    tvecs_str = np.array2string(tvecs, separator=',').replace("\n", "")

                    print("tvecs_str: " , tvecs_str)

                    msg += "tvecs:" + tvecs_str + "$" 
                    msg += "rvecs:" + rvecs_str + "#" 

                msg = msg[:-1]  # Remove the last character from the string
                print("Tag: " , msg)
